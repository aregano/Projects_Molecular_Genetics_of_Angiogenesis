...
# Aternative Dotplot for Specific Genes 6I.3
...
Top3genes_GeneSets_Conditions <- c(E2F_genes_Condition[[1]][[1]][1:3], E2F_genes_Condition[[2]][[1]][c(2:4)],
                                   Myc_genes_Condition[[1]][[1]][1:3], Myc_genes_Condition[[2]][[1]][c(2, 4, 5)],
                                   OxPhos_genes_Condition[[1]][[1]][1:3], OxPhos_genes_Condition[[2]][[1]][c(1,5,8)])
...
cairo_pdf("Plots/Figure_6/Figure_6I.3_DotPlot_GeneSets.pdf", width = 9.5, height = 2.2, family = "Arial")
DotPlot(Liver, features = DotPlot_list_caps, col.min = 0, dot.scale = 5, scale = T)+
  theme(strip.text.x = element_blank(), axis.title.x = element_blank(), axis.title.y = element_blank(), axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, face = "italic"),
        legend.title = element_text(size = 10), legend.text  = element_text(size = 8),
        legend.key.size = unit(0.3, "cm"), 
  ...
  scale_x_discrete(label = DotPlot_list)+
  scale_colour_gradientn(colours = Bestholtz_palette)+
  geom_vline(xintercept = c(6.5, 12.5, 18.5, 22.5, 26.5),linetype = 2 )+
  theme(legend.box = "horizontal")
dev.off()
...
# Figure S4.4G Violin Plot

genes <- list(c("MYC", "DLL4"))
genes <- as.data.frame(genes)
gene.names <- list(c("Myc", "Dll4"))
...
plegend <- VlnPlot(Liver, features = genes[i, 1], group.by = "Condition") +
  scale_fill_manual(values= palette_Maca_Vln, labels=c("Control", expression(italic("Dll4"^"iDEC")), expression(italic(Dll4/Myc)^"iDEC")))
plegend
...
cairo_pdf("Plots/Figure_6/Supplementary/Figure_S6A_VlnPlot.pdf", width = 5, height = 6, family = "Arial")
p27
dev.off()

jpeg("Plots/Figure_6/Supplementary/Figure_S6A_VlnPlot.jpeg", width = 5, height = 6, units = 'in', res = 800)
p27
dev.off()
...
write.csv(ranksd4, "./Tables/GSEA_Fig6/ranks.Dll4vsCtrl.BP.csv")

write.csv(ranksd4myc, "./Tables/GSEA_Fig6/ranks.Dll4MycvsCtrl.BP.csv")
...
myfgsea <- list(fgseaResD4, fgseaResD4Myc)

condition_short <- c("Dll4KO", "Dll4MycKO")

names(myfgsea) <- condition_short

write.xlsx(myfgsea, file = "./Tables/fgsea_Sup6D_Condition_Dll4_v_Control_Dll4Myc_v_Control_GO_BP.xlsx")
...
write.xlsx(myfgsea.filter, file = "./Tables/fgsea_Sup6D_Condition_Dll4_v_Control_Dll4Myc_v_Control_GO_BP.filter.xlsx")
...
selected.GOBP <- read.xlsx("Tables/GSEA_Fig6/fgsea_Sup6D_Condition_Dll4_v_Control_Dll4Myc_v_Control_GO_BP.filterSelectednotOriginal.xlsx", sheet = 4, rowNames = F)

selected.GOBP <- unique(selected.GOBP)
...
GOBP <- list(dll4.selected, dll4myc.selected)

condition_short <- c("Dll4KO", "Dll4/MycKO")

names(GOBP) <- condition_short

write.xlsx(GOBP, "Tables/GSEA_Fig6/GOBP_Macarena_selected.xlsx")
...
selected.GOBP <- read.xlsx("Tables/GSEA_Fig6/GOBP_Macarena_selected_Dll4_Dll4Myc.xlsx", sheet = 3, rowNames = T)
...
cairo_pdf("Plots/Figure_6/Supplementary/Figure_S6D_GSEA_GOBP_Dll4vsControl_Dll4MyciDECvsControl_unscaled.pdf", width = 5, height = 10, family = "Arial")
