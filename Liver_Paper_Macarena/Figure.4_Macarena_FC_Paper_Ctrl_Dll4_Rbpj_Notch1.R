...
table(Liver@metadata$Condition)
...
# Figure 4F_v2. Violin Plot Wnt2

p21 <- VlnPlot(Liver, features = "WNT2", group.by = "Condition", cols = palette_Maca_Vln)+ NoLegend() + theme(text = element_text(family="Arial"), 
                                                                                                               axis.title.x=element_blank(), axis.text.x=element_blank(), plot.title = element_text(face = "bold.italic"), axis.title.y = element_blank())+
  ggtitle("Wnt2")
...
p27
cairo_pdf("Plots/Figure_4/Figure_4F_v2_VlnPlot_Wnt2.pdf", width = 6, height = 4, family = "Arial")
p27
dev.off()

jpeg("Plots/Figure_4/Figure_4F_v2_VlnPlot_Wnt2.jpeg", width = 6, height = 4, units = 'in', res = 800)
p27
dev.off()
...
Liver.wo.dll4 <- subset(Liver, subset = Condition == "Control" | Condition == "NOTCH1KO" | Condition == "RBPJKO")

genes <- list(c("CDKN1A", "TRP53", "CDKN2A"))
genes <- as.data.frame(genes)
gene.names <- list(c("Cdkn1a", "Trp53", "Cdkn2a"))
gene.names <- as.data.frame(gene.names)
palette_Maca_Vln.wo.dll4 <- c("#606060", "#05BE78", "#55A0FB")
...
for (i in 1:nrow(genes)) {
  ...
  p21 <- VlnPlot(Liver.wo.dll4, features = genes[i, 1], group.by = "Condition", cols = palette_Maca_Vln.wo.dll4)+ NoLegend() + theme(text = element_text(family="Arial"), axis.title.x=element_blank(), axis.text.x=element_blank(), axis.title.y=element_blank(), plot.title = element_text(face = "bold.italic"))+ggtitle(gene.names[i, 1])
...
plegend <- VlnPlot(Liver.wo.dll4, features = genes[i, 1], group.by = "Condition") +
  scale_fill_manual(values= palette_Maca_Vln.wo.dll4, labels=c("Control", expression(italic(Notch1)^"iDEC"), expression(italic(Rbpj)^"iDEC")))
plegend
...
cairo_pdf("Plots/Figure_4/Figure_4G_VlnPlot.pdf", width = 4, height = 9, family = "Arial")
p27
dev.off()

jpeg("Plots/Figure_4/Figure_4G_VlnPlot.jpeg", width = 4, height = 9, units = 'in', res = 800)
p27
dev.off()
...
genes <- list(c("ESM1", "VEGFA", "KDR"))
genes <- as.data.frame(genes)
gene.names <- list(c("Esm1", "Vegfa", "Kdr"))
gene.names <- as.data.frame(gene.names)
...
for (i in 1:nrow(genes)) {
  ...
  p21 <- VlnPlot(Liver, features = genes[i, 1], group.by = "FigClustering", cols = my_palette_Rui_colors_B)+ NoLegend() + theme(text = element_text(family="Arial"), axis.title.x=element_blank(), axis.text.x=element_blank(), axis.title.y=element_blank(), plot.title = element_text(face = "bold.italic"))+ggtitle(gene.names[i, 1])
...
plegend <- VlnPlot(Liver, features = genes[i, 1], group.by = "FigClustering", cols = my_palette_Rui_colors_B) 
plegend
legend <- get_legend(plegend + theme(legend.position = "bottom", legend.justification = "center", legend.direction = "horizontal", legend.text = element_text(size = 11)))
...
cairo_pdf("Plots/Figure_4/Figure_4H_VlnPlot.pdf", width = 12, height = 9, family = "Arial")
p27
dev.off()

jpeg("Plots/Figure_4/Figure_4H_VlnPlot.jpeg", width = 12, height = 9, units = 'in', res = 800)
p27
dev.off()
...
genesFeaturePlots <- c("GJA5", "LTBP4", "MSR1", "RSPO3", "WNT2", "MKI67", "TOP2A", "STMN1", "RPL32", "RPL10A", "MALAT1", "MEIS1", "KCNE3", "ODC1", "CTNNB1", "RPL21", "CD34")
...
SphaseI <- c("MCM2", "MCM3")
SphaseI <- as.data.frame(SphaseI)
Sphase_I_text <- c("Mcm2", "Mcm3")

myplots <- vector('list', nrow(SphaseI))

for (i in 1:nrow(SphaseI)) {
  
  p3 <- FeaturePlot(Liver, features = SphaseI[i, 1], order = T, pt.size = 1.2, slot = "data", combine = F)
  
  p4 <- lapply(X = p3, FUN = function(p) p + scale_colour_gradientn(colors = Bestholtz_palette) + ggtitle(bquote(~italic(.(Sphase_I_text[i])))))
...
cairo_pdf("Plots/Figure_4/Suplementary/Figure_S4.1_FeaturePlot.Sphase_I.pdf", width = 12, height = 6, family = "Arial")
p25
dev.off()

jpeg("Plots/Figure_4/Suplementary/Figure_S4.1_FeaturePlot.Sphase_I.jpeg", width = 12, height = 6, units = 'in', res = 800)
p25
dev.off()

# S-phase-II

SphaseII <- c("ESM1", "VEGFA")
SphaseII <- as.data.frame(SphaseII)
Sphase_II_text <- c("Esm1", "Vegfa")

myplots <- vector('list', nrow(SphaseII))

for (i in 1:nrow(SphaseII)) {
  
  p3 <- FeaturePlot(Liver, features = SphaseII[i, 1], order = T, pt.size = 1.2, slot = "data", combine = F)
  
  p4 <- lapply(X = p3, FUN = function(p) p + scale_colour_gradientn(colors = Bestholtz_palette) + ggtitle(bquote(~italic(.(Sphase_II_text[i])))))
...
cairo_pdf("Plots/Figure_4/Suplementary/Figure_S4.1_FeaturePlot.Sphase_II.pdf", width = 12, height = 6, family = "Arial")
p25
dev.off()

jpeg("Plots/Figure_4/Suplementary/Figure_S4.1_FeaturePlot.Sphase_II.jpeg", width = 12, height = 6, units = 'in', res = 800)
p25
dev.off()
...
p3 <- FeaturePlot(Liver, features = "CD34", order = T, pt.size = 1.2, slot = "data", combine = F)
...
# Bmx, Ltbp4, Msr1, Rspo3, Mki67, Rpl32, Rpl10a, Top2a, Mcm2

gene.names <- c("Bmx", "Ltbp4", "Msr1", "Rspo3", "Mki67", "Rpl32", "Rpl10a", "Top2a", "Mcm2", "Rab3b")
genes <- gene.names %>% toupper()
genes <- as.data.frame(genes)
gene.names <- as.data.frame(gene.names)
...
cairo_pdf("Plots/Figure_4/Suplementary/Figure_S4.C_VlnPlot.pdf", width = 6, height = 30, family = "Arial")
p27
dev.off()

jpeg("Plots/Figure_4/Suplementary/Figure_S4.C_VlnPlot.jpeg", width = 6, height = 30, units = 'in', res = 800)
p27
dev.off()

############################################################

# Figure S4.4H Violin Plot

# 

gene.names <- c("Esm1", "Vegfa")
genes <- gene.names %>% toupper()
genes <- as.data.frame(genes)
gene.names <- as.data.frame(gene.names)
...
VlnPlot(Liver, features = c("ESM1", "VEGFA"), group.by = "FigClustering", cols = my_palette_Rui_colors_B, fill.by = "ident", stack = T)

p21 <- VlnPlot(Liver, features = genes[1, 1], group.by = "FigClustering", cols = my_palette_Rui_colors_B)+ NoLegend() + theme(text = element_text(family="Arial"), axis.title.x=element_blank(), axis.text.x=element_blank(), axis.title.y=element_blank(), plot.title = element_text(face = "bold.italic"))+ggtitle(gene.names[1, 1])

p22 <- VlnPlot(Liver, features = genes[2, 1], group.by = "FigClustering", cols = my_palette_Rui_colors_B)+ NoLegend() + theme(text = element_text(family="Arial"), axis.title.x=element_blank(), axis.title.y=element_blank(), plot.title = element_text(face = "bold.italic"), axis.text.x = element_text(angle = 90, vjust = 0.5))+ggtitle(gene.names[2, 1])
...
p24 <- patchwork::wrap_plots(p23, ncol = 1)

p24

p25 <- p24 + labs(title =  "Sepal length (cm)")

p25

x.axis <- ggdraw() + draw_label("Expression Level", fontface='bold', angle = 90, size = 20)

design <- c(patchwork::area(1, 1, 1, 1), patchwork::area(1, 2, 1, 20))

p26 <- x.axis + p25 + plot_layout(ncol = 2, design = design)

p26

cairo_pdf("Plots/Figure_4/Suplementary/Figure_S4.H_VlnPlot.pdf", width = 6, height = 8, family = "Arial")
p26
dev.off()

jpeg("Plots/Figure_4/Suplementary/Figure_S4.H_VlnPlot.jpeg", width = 6, height = 8, units = 'in', res = 800)
p26
dev.off()

############################################################

# Figure S4 Violin Plot Notch4 CtrlvNotch1
...
Liver <- subset(Liver, subset = Condition == "Control" | Condition == "NOTCH1KO")
...
for (i in 1:nrow(genes)) {
  ...
  p21 <- VlnPlot(Liver, features = genes[i, 1], group.by = "Condition", cols = c("#606060","#05BE78"))+ NoLegend() + theme(text = element_text(family="Arial"), axis.title.x=element_blank(), axis.text.x=element_blank(), axis.title.y=element_blank(), plot.title = element_text(face = "bold.italic"))+ggtitle(gene.names[i, 1])
...
plegend <- VlnPlot(Liver, features = genes[i, 1], group.by = "Condition") +
  scale_fill_manual(values= c("#606060","#05BE78"), labels=c("Control", expression(italic(Notch1)^"iDEC")))
plegend
legend <- get_legend(plegend + theme(legend.position = "bottom", legend.justification = "center", legend.direction = "horizontal", legend.text = element_text(size = 12)))

p25 <- patchwork::wrap_plots(myplots, ncol = 1)

p26 <- plot_grid(p25, legend, ncol = 1, rel_heights = c(1, .08))

x.axis <- ggdraw() + draw_label("Expression Level", fontface='bold', angle = 90, size = 14)

p27 <- plot_grid(x.axis, p26, ncol = 2, rel_widths = c(.14, 1))

p27

cairo_pdf("Plots/Figure_4/Suplementary/Figure_S4_Notch4.pdf", width = 3, height = 3, family = "Arial")
p27
dev.off()

jpeg("Plots/Figure_4/Suplementary/Figure_S4_Notch4.jpeg", width = 3, height = 3, units = 'in', res = 800)
p27
dev.off()
